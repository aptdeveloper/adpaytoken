...
export default class Login extends Component {
    ...
    render() {
        return <View style={styles.container}>
            <View style={{ alignItems: 'flex-end', marginTop: 40 * unitHeight }}>
    ...
    onCode() {
        if (ClickUtil.noDoubleClick()) {
            if (CommUtils.isTextEmpty(this.state.phonetel)) {
                return toastShort('手机号/邮箱不能为空')
            } else {
                if (CommUtils.isPhoneAvailable(this.state.phonetel) || CommUtils.isEmail(this.state.phonetel)) {
                    if (!this.state.textcode_flag) {
                        return;
                    }
                    this.setState({
                        textcode_flag: false
                    })
                    this.CodeApi(this.state.phonetel)
                } else {
                    return toastShort('请输入正确的手机号/邮箱')
                }
            }
        }
    }
    ...
    goback() {
        DeviceEventEmitter.emit('me', true)
        Actions.pop();
    }
    goForget_Password() {
